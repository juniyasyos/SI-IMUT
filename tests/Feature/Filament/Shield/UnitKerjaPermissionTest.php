...
describe('Unit Kerja Policies', function () {
    it('allows full policy access for authorized user', function () {
        ...
        expect($policy->view($this->userWithAccess, $this->unitKerja))->toBeTrue();
        expect($policy->create($this->userWithAccess))->toBeTrue();
        expect($policy->update($this->userWithAccess, $this->unitKerja))->toBeTrue();
        expect($policy->delete($this->userWithAccess, $this->unitKerja))->toBeTrue();
        expect($policy->deleteAny($this->userWithAccess))->toBeTrue();
        expect($policy->restore($this->userWithAccess, $this->unitKerja))->toBeTrue();
        expect($policy->restoreAny($this->userWithAccess))->toBeTrue();
        expect($policy->forceDelete($this->userWithAccess, $this->unitKerja))->toBeTrue();
    ...
    it('denies all policy access for unauthorized user', function () {
        ...
        expect($policy->view($this->userWithoutAccess, $this->unitKerja))->toBeFalse();
        expect($policy->create($this->userWithoutAccess))->toBeFalse();
        expect($policy->update($this->userWithoutAccess, $this->unitKerja))->toBeFalse();
        expect($policy->delete($this->userWithoutAccess, $this->unitKerja))->toBeFalse();
        expect($policy->deleteAny($this->userWithoutAccess))->toBeFalse();
        expect($policy->restore($this->userWithoutAccess, $this->unitKerja))->toBeFalse();
        expect($policy->restoreAny($this->userWithoutAccess))->toBeFalse();
        expect($policy->forceDelete($this->userWithoutAccess, $this->unitKerja))->toBeFalse();
